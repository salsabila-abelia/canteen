...
export class OrdersService {
  ...
  async createOrder(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!tenant) throw new NotFoundException('Kantin tidak ditemukan');
      ...
      if (pickupDate.toDateString() !== today.toDateString()) {
        throw new BadRequestException(
          'Pengambilan pesanan hanya bisa dilakukan pada hari ini',
        );
      }

      const pickupTimeStr = pickupDate.toTimeString().substring(0, 5); // Format "HH:MM"
      if (
        pickupTimeStr < tenant.open_time ||
        pickupTimeStr > tenant.close_time
      ) {
        throw new BadRequestException(
          `Waktu pengambilan harus antara jam ${tenant.open_time} - ${tenant.close_time}`,
        );
      }
      ...
      for (const item of items) {
        // Pessimistic Lock
        const products: any[] =
          await tx.$queryRaw`SELECT price, stock FROM Product WHERE id = ${item.productId} AND tenant_id = ${tenantId} FOR UPDATE`;
  ...
  async cancelOrder(userId: number, orderId: number) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Pesanan tidak dapat dibatalkan karena sudah diproses oleh kantin',
      );
    }
